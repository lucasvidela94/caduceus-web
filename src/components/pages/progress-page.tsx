...
import { Brain, BarChart3, Target, ArrowRight, Clock } from "lucide-react";

function getInsight(accuracy: number, total: number) {
	if (total === 0) return null;
	if (accuracy >= 80)
		return "¡Muy bien! Seguí así, estás construyendo buena base.";
	if (accuracy >= 60)
		return "Buen avance. Repasá las preguntas incorrectas para fortalecer áreas clave.";
...
export function ProgressPage() {
	...
	const insight = hasData
		? getInsight(progress.accuracy, progress.total_answered)
		: null;

	const lastSession = session?.completedAt ? session : null;
	const sessionAnswered = lastSession
		? Object.keys(lastSession.answers).length
		: 0;
		...
		? lastSession.questions.reduce(
				(acc, q) => {
					...
					if (!acc[q.category_id])
						acc[q.category_id] = { total: 0, correct: 0, name: "" };
		...
		<div className="mx-auto max-w-4xl space-y-8">
			<div>
				...
				<p className="mt-1 text-muted-foreground">
					Seguí tu evolución y encontrá áreas para mejorar.
				</p>
			</div>
						...
						<Card className="overflow-hidden">
							<div className="flex items-center gap-2 border-b border-border/40 bg-muted/50 px-5 py-3">
								...
								<span className="ml-auto text-xs text-muted-foreground">
									{sessionCorrect}/{sessionAnswered} correctas ·{" "}
									{sessionAccuracy}%
								</span>
							</div>
