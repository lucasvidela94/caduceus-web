...
function QuizEntry() {
  ...
  const isActive = session && !session.completedAt && session.currentIndex < session.questions.length
      ...
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <Card className="p-8 text-center">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm text-muted-foreground">
            Te mostramos 10 preguntas al azar. Respondé, aprendé con las explicaciones y seguí tu progreso.
          </p>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <Header />
      <ActiveSessionCard session={session} onContinue={() => navigate({ to: '/app/quiz/session' })} />
    </div>
  )
}
